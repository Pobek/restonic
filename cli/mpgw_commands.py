...
def create_mpgw(mpgw_name, fsh_name, backend_url, backend_type, request_type, response_type, policy_name, domain_name, state, xml_manager, host_rewrite, propagate_uri):
    ...
    if not any(request_type in req for req in request_respone_list):
        ...
        link = str(config["datapower_rest_url"]) + "config/"+ str(domain_name) +"/MultiProtocolGateway"
